...
if [ -n "$ADMIN_PASS" ];
    ...
    echo -e "Web app ${ACCENT}admin account POSSIBLY created ${NC}with username 'admin', login at <hostname>/admin"

    unset ADMIN_EMAIL
    unset ADMIN_PASS
fi

echo -e "Website using Person's table tuple with username ${ACCENT}${P_USERNAME}${NC}"
